<template>
  <div>
    ...
    <div
      class="flex flex-col justify-center items-center md:items-start md:flex-row gap-5 pt-5"
      v-else
    >
      <section class="flex-1 flex-col justify-start items-center">
        ...
        <div
          class="my-4 flex gap-3 flex-wrap items-center justify-between md:w-[90%] w-full"
        >
          <span class="font-style text-red-500">{{ recipeDetails.title }}</span
          ><span
            class="flex items-center font-Noto-San gap-2 shadow-lg text-xs rounded-md py-2 px-3 text-red-600"
            ><i class="pi pi-clock"></i>
            {{ recipeDetails.readyInMinutes }} mins</span
          >
        </div>
      </section>
      <section class="flex-1 font-Noto-San py-5 md:py-0">
        ...
        <ul class="flex gap-3 flex-wrap">
          <li
            class="py-2 px-4 hover:scale-105 hover:skew-x-2 transition-all cursor-pointer rounded-lg text-red-600 shadow-md"
            v-for="(ingredient, index) of recipeDetails.extendedIngredients"
          >
            {{ ingredient.name }}
          </li>
        </ul>
